...
- **Runtime**: Bun is installed at `~/.bun/bin/bun`. Ensure `$HOME/.bun/bin` is on `PATH` before running bun commands: `export PATH="$HOME/.bun/bin:$PATH"`.
- **Dev server**: `bun dev` starts Next.js 16 (Turbopack) on port 3000. Hot reload works automatically.
- **Required secret**: `YNS_API_KEY` must be set in `.env.local` (see `.env.example`). Without it, all Commerce Kit API calls fail with "Invalid API key". The build (`bun run build`) will also fail because it prerenders pages that call the API.
- **Type checking**: Use `npx @typescript/native-preview --noEmit` (aliased as `tsgo` in AGENTS.md). There is a pre-existing type error in `app/collection/[slug]/page.tsx` (`PageProps` not found) — this is in the repo, not something introduced by your changes.
- **Lint/format**: `bun run lint` runs Biome check with auto-fix. There is no separate `format` script; Biome handles both.
- **Tests**: No test files currently exist in the repo. `bun test` will report no tests found.
- **Pre-commit hooks**: Husky + lint-staged runs `bun biome check --write --unsafe --staged` on all staged files.
- **No database or Docker needed**: The app is a pure frontend that talks to the remote Commerce Kit API.
