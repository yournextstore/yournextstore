...
function Logo() {
	return (
		<YnsLink prefetch={"eager"} href="/" className="group flex flex-col items-center leading-none">
			<span className="font-marker text-3xl text-[#4b1fb5] transition-transform group-hover:-rotate-3">
				YNS
			</span>
			<span className="mt-0.5 text-[8px] font-bold uppercase tracking-[0.2em] text-[#1a0f4d]">
				Your Next Store
			</span>
		</YnsLink>
	);
	...
	return (
		<CartProvider initialCart={cart} initialCartId={cartId}>
			<div className="flex min-h-screen flex-col bg-background">
				<AnnouncementBar />
				<header className="sticky top-0 z-50 border-b border-[#ecd9c6] bg-white/95 backdrop-blur-md">
					<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
						<div className="grid grid-cols-3 items-center h-20">
							...
							<div className="flex items-center justify-center gap-6">
								<Suspense>
									<Navbar links={links} side="left-end" />
								</Suspense>
								<Logo />
								<Suspense>
									<Navbar links={links} side="right-start" />
								</Suspense>
							</div>
...
export default async function RootLayout({
	...
	return (
		<html lang={lang}>
			<body className={`${poppins.variable} ${permanentMarker.variable} antialiased`}>
