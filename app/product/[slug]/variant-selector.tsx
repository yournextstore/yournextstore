						...
						{group.type === "color" ? (
							<>
								<div className="mb-4 flex items-center justify-between">
									<legend className="text-xs tracking-[0.2em] uppercase text-zinc-400">{group.label}</legend>
									{selectedOption && <span className="text-xs text-zinc-500">{selectedOption.value}</span>}
								</div>
									...
									{group.options.map((option) => {
										...
										return (
											<button
												...
												title={option.value}
											>
												{isLightColor && <span className="absolute inset-0 border border-zinc-200" />}
											</button>
										);
									})}
								</div>
