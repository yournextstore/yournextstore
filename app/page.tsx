	...
	return (
		<section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
				{[
					{ src: "/scraped-4.jpg", label: "Living", href: "/collection/living" },
					{ src: "/scraped-7.jpg", label: "Dining", href: "/collection/dining" },
					{ src: "/scraped-13.jpg", label: "Bedroom", href: "/collection/bedroom" },
				].map((item) => (
					<YnsLink
						prefetch={"eager"}
						key={item.label}
						href={item.href}
						className="group relative overflow-hidden aspect-[2/3] transition-shadow duration-500 hover:shadow-xl"
					>
						<Image
							src={item.src}
							alt={item.label}
						...
						<div className="absolute bottom-6 left-6 text-white">
							...
							<span className="inline-flex items-center gap-1.5 text-sm opacity-80 mt-1 group-hover:opacity-100 transition-opacity">
								Shop All
...
function TopCollections() {
	const collections = [
		{ src: "/scraped-8.jpg", label: "Sofas", href: "/collection/sofas" },
		{ src: "/scraped-13.jpg", label: "Home Decor", href: "/collection/decor" },
		{ src: "/scraped-3.jpg", label: "Tables", href: "/collection/tables" },
		{ src: "/scraped-8.jpg", label: "Chairs", href: "/collection/chairs" },
	];

	return (
		<section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
			<h2
				className="text-[clamp(1.5rem,2.5vw,2rem)] font-medium text-center mb-12"
				style={{ fontFamily: "var(--font-heading)" }}
			>
				Top Collections
			</h2>
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
				{collections.map((col) => (
					<YnsLink prefetch={"eager"} key={col.label} href={col.href} className="group text-center">
						<div className="relative aspect-square overflow-hidden rounded-sm mb-4 transition-shadow duration-500 group-hover:shadow-lg">
							<Image
								src={col.src}
								alt={col.label}
								fill
			...
			<div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-10 items-start">
					<div className="relative aspect-[3/4] overflow-hidden">
						<Image src="/scraped-0.jpg" alt="Featured designer" fill className="object-cover" sizes="300px" />
						<div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
						<div className="absolute bottom-6 left-6 text-white">
...
function BrandLogos() {
	const brands = ["NORMANN", "MENU", "MUUTO", "FERM", "DONNA", "HAY", "FRITZ HANSEN"];

	return (
		<section className="border-y border-border py-10 sm:py-12 overflow-hidden">
			<div className="flex hover:[animation-play-state:paused] animate-[marquee_30s_linear_infinite]">
				{/* Duplicate for seamless loop */}
				{[...brands, ...brands].map((brand, i) => (
					<span
						key={`${brand}-${i}`}
						className="text-lg sm:text-xl font-medium tracking-widest text-foreground opacity-30 shrink-0 px-8 sm:px-12"
					>
						{brand}
					</span>
				))}
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
				{promos.map((promo) => (
					<YnsLink
						...
						key={promo.title}
						href={promo.href}
						className="group relative overflow-hidden aspect-[4/3] transition-shadow duration-500 hover:shadow-xl"
					>
						<Image
							src={promo.src}
							alt={promo.title}
						...
						<div className="absolute bottom-6 left-6 right-6 text-white">
							...
							<span className="inline-flex items-center gap-1.5 text-sm font-medium border-b border-white/50 pb-0.5 group-hover:border-white transition-all">
								{promo.cta}
				...
				{sections.map((section) => (
					<div key={section.title}>
						<div className="flex items-center gap-4 mb-6">
							<div className="relative w-14 h-14 rounded-full overflow-hidden shrink-0 ring-1 ring-border">
...
function Testimonials() {
	...
	return (
		<section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-10">
				{quotes.map((item) => (
					<div key={item.source} className="relative text-center px-4">
						<span
							className="absolute -top-2 left-1/2 -translate-x-1/2 text-7xl leading-none text-foreground/[0.06] pointer-events-none select-none"
							style={{ fontFamily: "var(--font-heading)" }}
							aria-hidden="true"
						>
							&ldquo;
...
function NewsletterSection() {
	return (
		<section className="bg-foreground text-primary-foreground py-20 sm:py-28">
			<div className="max-w-xl mx-auto px-4 text-center">
				<h2
					className="text-[clamp(1.5rem,2.5vw,2rem)] font-medium mb-4"
					style={{ fontFamily: "var(--font-heading)" }}
				>
					Give 20%, Get $20!
				</h2>
