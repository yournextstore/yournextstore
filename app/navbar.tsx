					...
					<nav className="mt-4 flex flex-col gap-1">
						{links.map((link) => (
							<YnsLink
								...
								className="rounded-lg px-3 py-3 text-sm font-extrabold uppercase tracking-[0.16em] text-yns-cocoa hover:text-yns-green transition-colors italic"
							>
								{link.label}
							</YnsLink>
						))}
			...
			<nav className="hidden lg:flex items-center gap-7">
				{links.map((link) => (
					<YnsLink
						...
						key={link.href}
						href={link.href}
						className="text-sm font-extrabold uppercase tracking-[0.16em] text-yns-cocoa hover:text-yns-green transition-colors italic"
					>
						{link.label}
					</YnsLink>
				))}
