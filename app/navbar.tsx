...
export function Navbar({ links }: { links: NavLink[] }) {
	...
	return (
		<>
			...
			<nav className="hidden sm:flex items-center gap-7 text-[12px] tracking-[0.18em] uppercase">
				{links.map((link) => (
					<YnsLink
						key={link.href}
						prefetch="eager"
						href={link.href}
						className="text-foreground/85 hover:text-[color:var(--oxblood)] transition-colors"
					>
						{link.label}
					</YnsLink>
				))}
