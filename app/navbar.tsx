...
export function Navbar({ links }: { links: NavLink[] }) {
	...
	return (
		<>
			...
			<nav className="hidden md:flex items-center gap-1 font-grotesk text-[11px] uppercase tracking-eyebrow text-muted-foreground">
				{links.map((link) => (
					<YnsLink
						key={link.href}
						prefetch="eager"
						href={link.href}
						className="group px-3 py-2 transition-colors hover:text-foreground"
					>
						{link.label}
					</YnsLink>
				))}
