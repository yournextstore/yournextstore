					...
					<nav className="mt-4 flex flex-col gap-1">
						{links.map((link) => (
							<YnsLink
								...
								className="rounded-lg px-3 py-3 text-[11px] font-mono uppercase tracking-[0.22em] text-foreground/70 hover:text-foreground transition-colors"
							>
								{link.label}
							</YnsLink>
						))}
			...
			<nav className="hidden md:flex items-center gap-7">
				{links.map((link) => (
					<YnsLink
						...
						key={link.href}
						href={link.href}
						className="text-[11px] font-mono uppercase tracking-[0.22em] text-foreground/70 hover:text-foreground transition-colors"
					>
						{link.label}
					</YnsLink>
				))}
