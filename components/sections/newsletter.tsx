...
import { subscribeToNewsletter } from "@/app/[locale]/newsletter/action";
				...
				<div className="max-w-2xl mx-auto text-center">
					{state?.success ? (
						<div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
							...
							<h2 className="text-2xl sm:text-3xl font-medium tracking-tight">{t("successTitle")}</h2>
							<p className="mt-3 text-background/60">{state.message}</p>
						</div>
					) : (
						<>
							<h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium tracking-tight">{t("title")}</h2>
