...
export function ChatPanel({
	...
	return (
		<div
			className={cn(
				"fixed inset-0 z-50 flex flex-col bg-background text-foreground shadow-2xl transition-all sm:inset-auto sm:bottom-20 sm:right-4 sm:h-[min(37.5rem,calc(100dvh-6.5rem))] sm:w-[25rem] sm:rounded-2xl sm:border",
		...
		>
			<header className="flex items-center justify-between border-b px-4 py-3">
				...
				<button
					...
					className="cursor-pointer rounded-full px-2 py-1 text-sm text-muted-foreground hover:bg-muted sm:hidden"
				>
					Close
				</button>
			</header>
		...
		<>
			{message.parts.map((part, index) => {
				if (part.type === "text" && part.text.trim()) {
					return <AssistantBubble key={index}>{part.text}</AssistantBubble>;
				}
...
function AssistantBubble({ children }: { children: React.ReactNode }) {
	return (
		<div className="mr-8 self-start rounded-2xl rounded-bl-md border bg-muted/40 px-3 py-2 text-sm whitespace-pre-wrap">
			{children}
		</div>
	);
}
