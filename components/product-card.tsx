	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group">
			...
			<div className="mt-4 space-y-1 text-center">
				<h3 className="font-serif text-base sm:text-lg text-ink tracking-tight">{product.name}</h3>
				{priceDisplay && <p className="text-sm text-ink/80 tracking-wide">{priceDisplay}</p>}
				{variantCount > 1 && (
					<p className="text-[10px] tracking-[0.22em] uppercase text-muted-foreground pt-1">
						{variantCount} colour{variantCount === 1 ? "" : "s"}
					</p>
				)}
			</div>
		</YnsLink>
