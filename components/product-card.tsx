...
export function ProductCard({
	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group block">
			<div className="relative aspect-[4/5] overflow-hidden rounded-sm bg-[#f5ede2]">
				<WishlistButton />
			...
			<div className="mt-4 space-y-1 px-1">
				<h3 className="text-[14px] font-medium leading-snug text-foreground">{product.name}</h3>
				{priceDisplay && (
					<p className="text-[13px] font-medium tracking-wide text-foreground/80">{priceDisplay}</p>
				)}
			</div>
		</YnsLink>
	);
