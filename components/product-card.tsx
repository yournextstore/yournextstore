	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group block">
			...
			<div className="pt-4 flex items-start justify-between gap-3">
				<h3 className="font-serif text-[18px] sm:text-[19px] font-light tracking-tight text-foreground leading-tight">
					{product.name}
				</h3>
				{priceDisplay && (
					<p className="shrink-0 text-[12px] text-foreground/80 tabular-nums tracking-wide">{priceDisplay}</p>
				)}
			</div>
		</YnsLink>
	);
