...
export function ProductCard({
	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group block">
			<div className="relative aspect-square bg-white/60 rounded-2xl overflow-hidden mb-4 border border-[#1b2a2e]/10">
				{singleVariant && (
					<QuickAddButton
			...
			<div className="space-y-1 px-1 pb-2">
				<h3 className="font-display text-lg font-bold text-[#1b2a2e] leading-tight">{product.name}</h3>
				<p className="text-sm font-semibold text-[#1b2a2e]/80">{priceDisplay}</p>
			</div>
		</YnsLink>
	);
}
