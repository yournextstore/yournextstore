...
export function ProductCard({
	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group block">
			<div className="relative aspect-[4/5] bg-secondary/70 overflow-hidden rounded-sm">
				{singleVariant && (
					<QuickAddButton
				...
				{number && (
					<span className="absolute top-3 left-3 editorial-eyebrow text-foreground/70 bg-background/70 backdrop-blur-sm px-2 py-1 rounded-sm">
						N°{number}
					</span>
				)}
			</div>
			<div className="mt-5 flex items-start justify-between gap-4">
				<div>
					<h3 className="font-display italic text-xl text-foreground leading-tight tracking-tight">
						{product.name}
					</h3>
					<p className="mt-1 editorial-eyebrow text-muted-foreground">In stock · ships in 48h</p>
				</div>
				<p className="shrink-0 text-sm font-medium tabular-nums text-foreground pt-1">{priceDisplay}</p>
			</div>
		</YnsLink>
	);
}
