...
export function ProductCard({
	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group block">
			<div className="relative aspect-square bg-bone-gradient rounded-sm overflow-hidden mb-5 ring-1 ring-border/60">
				{singleVariant && (
					<QuickAddButton
			...
			<div className="flex items-baseline justify-between gap-4">
				<h3 className="font-serif text-xl leading-tight tracking-tight text-foreground">{product.name}</h3>
				<p className="shrink-0 text-[13px] text-muted-foreground tabular-nums">{priceDisplay}</p>
			</div>
		</YnsLink>
	);
}
