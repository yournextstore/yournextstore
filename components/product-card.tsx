...
export function ProductCard({
	...
	return (
		<YnsLink prefetch={"eager"} href={`/product/${product.slug}`} className="group block">
			<div className="relative aspect-square bg-[var(--sand)]/50 rounded-2xl overflow-hidden mb-4 border border-border">
				{singleVariant && (
					<QuickAddButton
			...
			<div className="flex items-baseline justify-between gap-3 px-1">
				<h3 className="text-[13px] tracking-wide text-muted-foreground truncate">{product.name}</h3>
				<p className="text-sm font-medium text-foreground shrink-0 tabular-nums">{priceDisplay}</p>
			</div>
		</YnsLink>
	);
}
